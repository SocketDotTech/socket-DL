...
const gasPrice = 25;
const url =
    `https://api.polygonscan.com/api?module=account&action=txlist&address=${deployerAddr}&sort=asc&apikey=${apiKey}${startBlock ? '&startBlock=' + startBlock : ''}${endBlock ? '&endBlock=' + endBlock : ''}`
