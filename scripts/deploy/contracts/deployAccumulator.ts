...
export default async function deployAccumulator(socketAddress: string, notaryAddress: string, remoteChainSlug_: number, signer: SignerWithAddress) {
  try {
    const contractName = "SingleAccum";
    const args = [socketAddress, notaryAddress, remoteChainSlug_]
